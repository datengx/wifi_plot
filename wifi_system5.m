...
for i = 1:length(basement_ap_ids)
    curr_data = unique_data(unique_data(:,2)==basement_ap_ids(i), :);
    curr_data(:,2)=i;
    temp_data = [temp_data; curr_data];
end
unique_data = temp_data;

xyz=[];
for i =1:length(basement_ap_ids)
    curr_data = unique_data(unique_data(:,2)==i,:);
    [~,ind] = max(curr_data(:,3));
    xyz = [xyz; curr_data(ind,6:7)'; 1.2];
end

n_A = [3.5, 40];
locations = unique(unique_data(:,1));

err = 1e12;
prev_err = 1e13;
while err>1e-3 && (prev_err-err)>1e-3
    J = zeros(length(locations),3*length(basement_ap_ids));
    r = zeros(length(locations),1);
    for i=1:length(locations)
        j = locations(i);
        curr_data = unique_data(unique_data(:,1)==j,:);
        d = 10.^((n_A(2)-curr_data(:,3)-20*log10(curr_data(:,4))) / 10 / n_A(1));
        
        for k =1:length(d)
            l=curr_data(k,2);
            pos = xyz(3*l-2:3*l);
            J_curr = [2*(pos(1)-curr_data(k,6)), 2*(pos(2)-curr_data(k,7)), 2*pos(3)];
            J(i,3*l-2:3*l) = J_curr;
            r_curr = (pos(1)-curr_data(k,6))^2 + (pos(2)-curr_data(k,7))^2 + pos(3)^2 - d(k)^2;
            r(i) = r(i) + r_curr;
        end
        r(i)=r(i);
    end
...
%     dxyz = (J'*W*J)\(J'*W*r);
    ...
    for k = 1:size(unique_data,1)
        l = unique_data(k,2);
        unique_data(k,9:11) = xyz(3*l-2:3*l)';
    end
    ...
    n_A = [D,-ones(size(unique_data,1),1)] \ (-R-20*log10(unique_data(:,4)))
end
